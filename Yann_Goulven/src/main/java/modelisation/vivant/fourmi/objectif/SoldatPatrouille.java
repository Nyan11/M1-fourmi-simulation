...
public class SoldatPatrouille implements Objectif{

	private Objectif objectifRetour;
	private boolean decisionFaim;
	
	public SoldatPatrouille(Objectif objectifRetour) {
	...
	public boolean aValiderObjectif(Terrain terrain, Adulte adulte) {
		return adulte.proieVisible(terrain) || this.aFaim(adulte.getTempsSansManger(), adulte.TEMPS_POUR_MANGER);
	}
	...
	public Objectif objectifSuivant(Terrain terrain, Adulte adulte) {
		...
		} else if (adulte.proieVisible(terrain)) {
			Position prochaine = adulte.getDeplacement().deplacementVersProie(terrain);
			adulte.setProchainePosition(prochaine);
			adulte.lacherPheromone(terrain);
			return new ChasseAttaque(objectifRetour);
		} else {
			return new SoldatPatrouille(objectifRetour);
		}
	}
